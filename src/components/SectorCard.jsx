        ...
        <div style={{ padding: 16 }}>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 10 }}>
            <div style={{ display: "flex", gap: 10 }}>
              <div style={{ width: 32, height: 32, borderRadius: 10, background: "#f0fdfa", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 16, flexShrink: 0 }}>📍</div>
              <div>
            ...
            <div style={{ display: "flex", gap: 6, alignItems: "center" }}>
              ...
              <button onClick={e => { e.stopPropagation(); setMenu(m => !m); }} style={{ background: "none", border: "none", cursor: "pointer", color: "#94a3b8", padding: 2 }}>⋯</button>
            </div>
          </div>
          ...
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginTop: 10, paddingTop: 10, borderTop: "1px solid #f8fafc" }}>
            ...
            <button onClick={e => { e.stopPropagation(); onActivity(sector); }}
              style={{ fontSize: 11, color: "#0d9488", fontWeight: 700, background: "none", border: "none", cursor: "pointer" }}>+ Activitate</button>
          </div>
        </div>
        {menu && (
          <div ref={menuRef} onClick={e => e.stopPropagation()} style={{ position: "absolute", top: 44, right: 8, background: "#fff", borderRadius: 12, boxShadow: "0 4px 20px rgba(0,0,0,.12)", border: "1px solid #f1f5f9", zIndex: 50, minWidth: 150 }}>
            <button onClick={() => { setMenu(false); onEdit(sector); }} style={{ display: "flex", gap: 8, alignItems: "center", width: "100%", padding: "9px 12px", background: "none", border: "none", cursor: "pointer", fontSize: 12, color: "#334155" }}>✏️ Editează</button>
            <button onClick={() => { setMenu(false); onActivity(sector); }} style={{ display: "flex", gap: 8, alignItems: "center", width: "100%", padding: "9px 12px", background: "none", border: "none", cursor: "pointer", fontSize: 12, color: "#334155" }}>➕ Activitate</button>
            <hr style={{ border: "none", borderTop: "1px solid #f1f5f9", margin: "4px 0" }} />
            <button onClick={() => { setMenu(false); setDel(true); }} style={{ display: "flex", gap: 8, alignItems: "center", width: "100%", padding: "9px 12px", background: "none", border: "none", cursor: "pointer", fontSize: 12, color: "#dc2626" }}>🗑 Șterge</button>
          </div>
        )}
      </div>
